...
void OMElement::init(OMNode * parent, std::string localname, OMNamespace * ns)
{
    ...
    if(ns)
    {
        ...
        _namespace = new OMNamespace(ns_c);
        _added_namespaces.push_back(_namespace);
    }
...
OMElement::OMElement(OMNode * parent, axiom_node_t * node)
{
    ...
    children_iterator = axiom_element_get_children(_wsf_axiom_element, Environment::getEnv(), node);
    if (children_iterator)
    {
        while (axiom_children_iterator_has_next(children_iterator, Environment::getEnv()))
        {
            ...
            axiom_types_t n_type = axiom_node_get_node_type(node_new, Environment::getEnv());
            if (n_type == AXIOM_ELEMENT)
            {
...
{
    std::string str_uri;
    if (ns) str_uri = ns->getURI();

    std::vector<OMAttribute*>::iterator attrs_end = _added_attributes.end();
    for (vector<OMAttribute *>::iterator i = _added_attributes.begin();
        i != attrs_end; ++i)
    {
        /* Compare ns first as it is smaller in size */
        OMNamespace *ns_i = (*i)->getNamespace();
        if (((ns_i == NULL) || ((ns_i != NULL) && (ns_i->getURI() == str_uri)))
            && ((*i)->getName() == name))
        {
            return (*i);
        }
...
OMElement * OMElement::getFirstElement()
{
    for (vector<OMNode *>::iterator i = _child_nodes.begin();
        i < _child_nodes.end(); ++i)
    {
        if ((*i)->nodeType() == AXIOM_ELEMENT)
            return (static_cast<OMElement*>(*i));
    }
    return NULL;
}
...
bool OMElement::setNamespace(OMNamespace * ns)
{
    ...
    if (status == AXIS2_SUCCESS)
    {
        _added_namespaces.push_back(new OMNamespace(ns->getAxiomNamespace()));
        return true;
    }
    return false;
}
