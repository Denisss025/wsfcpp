...
OMNode * OMNode::getPreviousSibling()
{
	...
	std::vector<OMNode*>::iterator i =
		std::find(children.begin(), children.end(), this);
	if ((i == children.begin()) || (i == children.end())) return (NULL);
	return (*(--i));
}
...
OMNode * OMNode::getNextSibling()
{
	...
	std::vector<OMNode*> children = dp->getChildren();
	std::vector<OMNode*>::reverse_iterator i =
		std::find(children.rbegin(), children.rend(), this);
	if ((i == children.rbegin()) || (i == children.rend())) return (NULL);
	return (*(--i));
}
